...
2. If the file is empty or doesn't exist, try fetching the JD from `{{URL}}` via WebFetch
...
Read `cv.md`. Table mapping each JD requirement to exact lines from the CV or i18n.ts keys.
...
**Step 1 — WLB from CSV (zero token cost):**
Read `companies_merged.csv`. Search for the company by name (case-insensitive) in the `name` column.
If found, use `glassdoor_ds_it_wlb_score` as the WLB score for the role.
Note: "WLB (Glassdoor DS/IT): X.X/5 — source: companies_merged.csv"
If company not in CSV, use WebSearch for WLB.

**Step 2 — Comp via WebSearch:**
Current salaries (Levels.fyi, Blind, Glassdoor), company comp reputation, demand trend.
